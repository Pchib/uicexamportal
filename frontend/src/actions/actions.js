...
const asynchorousFetchUserDB = (email, password) => {
  ...
  return (dispatch) => {
    dispatch(biginFetch());
    fetch("https://uicexamportal.herokuapp.com/auth/login", {
      method: "POST",
...
const asynchorousSendAnswer = (questionPayload) => {
  ...
  return (dispatch) => {
    ...
    fetch("https://uicexamportal.herokuapp.com/api/exam", {
      ...
      .then((res) => {
        console.log(res);
        if (res.status) dispatch(answerSendSucceed(res.score));
        else dispatch(answerSendFail({ error: res.error }));
      })
      .catch((error) => {
        console.log(error);
